...
const ROUTE_BOTTOM = SIZE - LOGO_HEIGHT;
...
export function ShareRouteImage({
  coordinates,
  territory,
  territoryArea,
  distance,
  duration,
}: ShareRouteImageProps) {
  const shapes = useMemo(() => {
    ...
    const toX = (lng: number) => ((lng - bMinLng) / lngSpan) * SIZE;
    const toY = (lat: number) => ((bMaxLat - lat) / latSpan) * ROUTE_HEIGHT + ROUTE_TOP;
      ...
      <Svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
        <Defs>
          <ClipPath id="routeClip">
            <Rect x={0} y={ROUTE_TOP} width={SIZE} height={ROUTE_HEIGHT} />
          </ClipPath>
        </Defs>
        ...
        {/* Distance above logo */}
        <SvgText x={SIZE / 2} y={SIZE - 80 - 60 * 1.0 - 50} textAnchor="middle">
        ...
        {/* Logo */}
        <LogoPaths logoWidth={262} logoScale={1.0} logoY={SIZE - 80 - 60 * 1.0} />
      </Svg>
    </View>
  );
}
...
const LogoPaths = ({
  ...
  <>
    <Path
      ...
      fill={Brand.white}
      transform={`translate(${(SIZE - logoWidth * logoScale) / 2}, ${logoY}) scale(${logoScale})`}
    />
    <Path
